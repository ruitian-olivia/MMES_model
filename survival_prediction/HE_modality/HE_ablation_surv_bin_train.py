...
from HE_ablation_surv_models import HE_graph_hypergraph_surv, HE_features_hypergraph_surv, HE2_concat_surv
...
class HE_CoxGraphDataset(Dataset):
    def __init__(self, surv_df):
        super(HE_CoxGraphDataset, self).__init__()
    ...
    def get(self, idx):
        ...
        HE_data_origin = torch.load(match_item['sub_HE_graph_path'].tolist()[0]) 
        HE_features_df = pd.read_csv(match_item['sub_HE_features_path'].tolist()[0])
        HE_features_np = HE_features_df.to_numpy()

        data = Data(x=HE_data_origin.x, edge_index=HE_data_origin.edge_index)
        data.survival = torch.tensor(survival)
        data.censorship = torch.tensor(censorship)
        data.surv_label = torch.tensor(surv_label)
        data.item = item
        data.pos_HE = HE_data_origin.pos
        data.HE_features = torch.tensor(HE_features_np, dtype=torch.float)
...
def Train(Argument):
    ...
    surv_csv_path = '../../preprocessed_data/clinical_data/surv_bin_HE_Virchow_patches_df.csv'
    ...
    HE_TrainDataset = HE_CoxGraphDataset(surv_df=surv_disc_df)
    HE_TestDataset1 = HE_CoxGraphDataset(surv_df=surv_test1_df)
    HE_TestDataset2 = HE_CoxGraphDataset(surv_df=surv_test2_df)
       
    train_loader = DataLoader(HE_TrainDataset, batch_size=batch_num, shuffle=True, num_workers=1, pin_memory=True)
    test1_loader = DataLoader(HE_TestDataset1, batch_size=batch_num, num_workers=1, pin_memory=True, drop_last=False)
    test2_loader = DataLoader(HE_TestDataset2, batch_size=batch_num, num_workers=1, pin_memory=True, drop_last=False)

    if Argument.GNN_type == 'HE1_graph':
        model = HE_graph_hypergraph_surv(surv_bins=Argument.surv_bins,
                            HE_nuclei_num=Argument.HE_nuclei_num,
                            HE_dim_target=Argument.HE_dim_target,
                            layers=Argument.layers, 
                            surv_mlp=Argument.surv_mlp,
                            dropout=Argument.dropout_rate)

    elif Argument.GNN_type == 'HE1_features':
        model = HE_features_hypergraph_surv(surv_bins=Argument.surv_bins,
                            HE_features_num=Argument.HE_features_num,
                            HE_dim_target=Argument.HE_dim_target,
                            surv_mlp=Argument.surv_mlp,
                            dropout=Argument.dropout_rate)

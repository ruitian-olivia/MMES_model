...
def train_surv_bin(model,train_loader,optimizer,surv_loss):
    ...
    with torch.set_grad_enabled(grad_flag):
        ...
        for index, HE_data in enumerate(train_loader,1):
            optimizer.zero_grad()          

            tempsurvival = HE_data.survival  # 单个batch数据中的survival data
            tempcensor = HE_data.censorship
            templabel = HE_data.surv_label
            
            HE_tempID = np.asarray(HE_data.item)
...
def patient_test_surv_bin(model, test_loader):
    ...
    with torch.set_grad_enabled(grad_flag):
        ...
        for index, HE_data in enumerate(test_loader,1):
        
            tempsurvival = HE_data.survival 
            tempcensor = HE_data.censorship
            templabel = HE_data.surv_label
